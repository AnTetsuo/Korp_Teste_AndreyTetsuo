...
{
    private readonly IDbContextOutbox<InvoicingDbContext> _outbox;

    public WolverineOutbox(IDbContextOutbox<InvoicingDbContext> outbox, InvoicingDbContext context)
    {
        if (outbox.DbContext is not null && !ReferenceEquals(outbox.DbContext, context))
            throw new InvalidOperationException(
                "Wolverine's outbox is wrapping a different InvoicingDbContext instance than the " +
